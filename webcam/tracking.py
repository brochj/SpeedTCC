...
class Tracking:
    ...
    def __create_tracked_blobs_dict(self, center):
        ...
        if not self.closest_blob:
            ...
            self.tracked_blobs.append(b)  # Now tracked_blobs wont be False anymore
    ...
    def remove_expired_track(self, blob_track_timeout, name, frame_time):
        if self.tracked_blobs:
            ...
            for i in range(len(self.tracked_blobs) - 1, -1, -1):
                if frame_time - self.tracked_blobs[i]['last_seen'] > blob_track_timeout: # Deleta caso de timeout
                    print("Removing expired track from {} | id: {} |".format(name, self.tracked_blobs[i]['id']))
