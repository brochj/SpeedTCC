...
kernel2 = np.ones((6,6), np.uint8) # # Matriz (8,8) com 1 em seus valores -- Usa na funcao de dilate
...
while(True):
    ...
    if ret == True:
        ...
        fgmask1 = cv2.morphologyEx(fgmask, cv2.MORPH_OPEN, kernel1)
        fgmask2 = cv2.morphologyEx(fgmask1, cv2.MORPH_CLOSE, kernel2)
        
        erodedmask = cv2.erode(fgmask2, kernel1, iterations=1) # usa pra tirar os pixels isolados (ruídos)
